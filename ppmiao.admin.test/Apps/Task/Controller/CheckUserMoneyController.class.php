...
class CheckUserMoneyController extends Controller {
    ...
    public function main2(){
        ...
        $error = [];
        $users = $this->getUserDetail();
        $body = '<table  border="1"><tr><td>用户ID</td><td>姓名</td><td>存管冻结值</td><td>存管查询值</td></tr>';
        foreach($users as $user){
            $detail = $this->getUserFundMoney($user['platcust']);
            if($detail->result->frozen_amount < 0){


                \Think\Log::write('异常数据:'.'<tr><td>'.$user['id'].'</td><td>'.$user['real_name'].'</td><td>'.$detail->result->frozen_amount.'</td><td>'.$detail->result->balance.'</td></tr>');
                $error[$user['id']]['database'] = $user['wallet_totle'];
                $error[$user['id']]['fund'] = $detail->result->balance;
                $body .='<tr><td>'.$user['id'].'</td><td>'.$user['real_name'].'</td><td>'.$detail->result->frozen_amount.'</td><td>'.$detail->result->balance.'</td></tr>';
            }
        }
        $body .= '</table>';
        $this->sendEmail($body);
    ...
    public function getUserDetail(){
        $sql = "select u.id,u.real_name,u.platcust,a.wallet_totle  from s_user_account as a left join s_user as u on u.id = a.user_id where u.platcust <> ''  and u.id in (select user_id from s_user_bank where main_card = 1 and is_deleted = 0)";
