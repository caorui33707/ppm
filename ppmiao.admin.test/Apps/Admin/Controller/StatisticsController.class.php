...
class StatisticsController extends AdminController{
    ...
    protected function _initialize(){
        ...
        $this->statisticsModel = new \Admin\Model\StatisticsModel();
    }
    ...
    public function channel_add(){
        if(!IS_POST){
            ...
            $cons_type = I('post.cons_type', -1, 'int');
            $open_num  = I('post.open_num', 0, 'int');
            $status    = I('post.status', 0, 'int');
            $ver       = trim(I('post.ver', '', 'strip_tags')); // 渠道号

            if(!$name) $this->ajaxReturn(array('status'=>0,'info'=>'渠道名称不能为空'));
            if(!$sign) $this->ajaxReturn(array('status'=>0,'info'=>'包名不能为空'));
            //if(!$address) $this->ajaxReturn(array('status'=>0,'info'=>'地址不能为空'));
            if(!$open_num) $this->ajaxReturn(array('status'=>0,'info'=>'打开人数不能为空'));
            if(!$ver) $this->ajaxReturn(array('status'=>0,'info'=>'渠道号不能为空'));
    ...
    public function channel_statistics(){
        if(!IS_POST){
            ...
            $channelArr  = $channelKeyArr = array();



            foreach ($channelList as $c){
                $channelArr[$c['id']] = $c['cons_value']; // .'('.$c['cons_key'].')'
                $channelKeyArr[$c['id']] = $c['cons_key'];
            }

            $this->assign('cons_type', $cons_type);
            $this->assign('channel_list', $channelList);

            $userWalletRecordsObj = M('UserWalletRecords'); // 回款记录
            ...
            if($doSearch){
                ...
                if(!$chn){ // 全部渠道
                   ...
                   // $cond[] = "user_id in (select id from s_user where channel_id=".$channelList[$i]['id'].")";
                    ...
                    //当日注册用户数
                    $sql =" select * from (select id ,id as user_id,add_time,substr(add_time,1,10) as group_add_time,
                            0 as due_capital,
                            3 as frm from s_user where id>0 and add_time>='" . $start_time . " 00:00:00.000000' and add_time<='" . $end_time . " 23:59:59.999000' ";
                    $sql .= "UNION ";
                    //当日注册用户数
                    $sql .="select id ,user_id,add_time,substr(add_time,1,10) as group_add_time,
                            0 as due_capital,
                            3 as frm from s_user_channel where user_id>0 and add_time>='" . $start_time . " 00:00:00.000000' and add_time<='" . $end_time . " 23:59:59.999000' ";
                    //还款
                    $sql .= "UNION ";
                    $sql .= " select id ,user_id,add_time,substr(add_time,1,10) as group_add_time,
                            0 as due_capital,
                            4 as frm from s_user_wallet_records where user_id>0 and type=4 and add_time>='" . $start_time . " 00:00:00.000000' and add_time<='" . $end_time . " 23:59:59.999000' ";

                    // 投资
                    $sql .= "UNION ";
                    $sql .= " select id,user_id,add_time,substr(add_time,1,10) as group_add_time,
                              due_capital ,
                              1 as frm from s_user_due_detail where user_id>0 and add_time>='" . $start_time . " 00:00:00.000000' and add_time<='" . $end_time . " 23:59:59.999000' ";



                    $sql .= " ) as t order by group_add_time,id";

                    $userPayList =  M()->query($sql);
                    //print_r($sql);


//                    $userPayList = $rechargeLogObj->
//                    field('id ,user_id,add_time,substr(add_time,1,10) as group_add_time,
//                    amount,recharge_no,
//                    1 as frm')->
//                    union($sql)->
//                    where($conditions)->select(); //->order('add_time')

                    $channelUserArr = array();
                    ...
                    // 某几个 uid 下的 channel_id
                    $sql = 'select * from  ( ';
                    $sql .=" select u.user_id as id ,c.id as channel_id from s_user_channel u left join  s_constant c on c.cons_key=u.channel where u.user_id in ( ". implode(',',$channelUserArr) ." )";
                    $sql .=" union ";
                    $sql .= " select  id  ,channel_id from s_user where id in ( ". implode(',',$channelUserArr) ." )";
                    $sql .= " ) as t  " ;

                    $userChannelArr = M()->query($sql);
                    ...
                    foreach ($userPayList as  $group){
                        ...
                        if(!empty($groupAddTime) && !empty($chnId) ) {
                            ...
                            $group['cons_key'] = $channelKeyArr[$chnId];
                    ...
                    for($i = $min; $i < $max; $i++){
                       ...
                       // $list[$pos] = $userPayListRow[$i]; // 渠道信息
                        ...
                        $list[$pos]['cons_value'] = $cons_value =!empty($cons_value_one = $userPayListRow[$i]['cons_value'])?$cons_value_one:'未知';
                        $cons_key =!empty($cons_key_one = $userPayListRow[$i]['cons_key'])?$cons_key_one:'未知';
                        ...
                        $condWalle[] = "user_id>0";
                        $condWalle[] = "type=4"; //待收状态  已还款

                        if($now_time) $condWalle[] = "add_time>='".$now_time." 00:00:00.000000'";
                        if($now_time) $condWalle[] = "add_time<='".$now_time." 23:59:59.999000'";

                        $conditionsWalle = implode(' and ', $condWalle);


                        $userDueDetailArr = M('user_wallet_records')->field('value ,user_id')->where($conditionsWalle)->order('add_time')->select();

                        $totalRepaymentList = array();
                        $totalRepaymentUser = $totalRepaymentUserPaySum = 0;
                        foreach ($userDueDetailArr as $due) {
                            $dueAmount = isset($due['value']) ? trim($due['value']) : 0;
                        ...
                        unset($userPayList['channel_id'],$userPayList['cons_value'],$userPayList['group_add_time'],$userPayList['cons_key']);


                        foreach($userPayList as $key => $val){

                            if($val['frm']!=1) {
                                unset($userPayList[$key]);
                                continue;
                            }
                           ...
                           // $bagAmount = M('UserRedenvelope')->where(array('recharge_no'=>$val['recharge_no'],'user_id'=>$val['user_id'],'status'=>1))->getField('amount');
                            ...
                            if(!in_array($val['user_id'], $firstUserList)){ // 是第一次购买用户ID
                                ...
                                $userDueDetailWhere = " and add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'";
                                if(!$userDueDetailObj->where('user_id='.$val['user_id'].' and status=2 and id<'.$val['id'] . $userDueDetailWhere)->getField('id')) { // 检查这个时间点外该用户是否有购买(没有)
                                    $firstPayPersonSum += 1;

                                    $totalPayPersonSum += 1; // 总投资用户数
                            ...
                            }else if($arrayCount[$val['user_id']]==1){ //是第二次购买用户ID
                                array_push($firstUserList, $val['user_id']);
                            ...
                            if(isset($val['user_id'])) {
                                array_push($totalUserList, $val['user_id']);//所有 userId
                            }

                            $totalPaySum += $val['due_capital'];

                            $arrayCount = array_count_values ($totalUserList);
...
//                        $unionSql = "select count(id) as tp_count   from s_user_channel where channel LIKE '".$cons_key."%' and  user_id>0 and add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000' LIMIT 1 ";
//
//                        $list[$pos]['user_count'] = $userObj->union($unionSql)->where($userConditions)->count();

                        $sqlChn = "  id in (select id from s_user where channel_id=" . $chnId . " OR id IN (SELECT user_id FROM s_user_channel WHERE channel LIKE '".$cons_key."%' ) ) ";

                        if(in_array($cons_key,$this->consKeyArray)){ // 特殊处理
                            $sqlChn = "  id in (select id from s_user where channel_id=" . $chnId . " OR id IN (SELECT user_id FROM s_user_channel WHERE channel = '".$cons_key."' ) ) ";
                        }

                        $list[$pos]['user_count'] = $userObj->where( $sqlChn . " and add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'")->count();
...
//                        $list[$pos]['user_count'] = $userObj->where(array('channel_id'=>$channelList[$i]['id']))->count();
                   ...
                   // $groupUserPayList = $rechargeLogObj->field('substr(add_time,1,10) as group_add_time,s_recharge_log.*')->where($conditions)->order('add_time')->select();
                    ...
                    $cons_key = $channelKeyArr[$chn];
                    $sqlChn = " where  user_id in (select id from s_user where channel_id=" . $chn . " OR id IN (SELECT user_id FROM s_user_channel WHERE channel LIKE '".$cons_key."%' ) ) ";

                    if(in_array($cons_key,$this->consKeyArray)){ // 特殊处理
                        $sqlChn = " where  user_id in (select id from s_user where channel_id=" . $chn . " OR id IN (SELECT user_id FROM s_user_channel WHERE channel = '".$cons_key."' ) ) ";
                    }
                    ...
                    $sql .= " ) as t  " .$sqlChn . " order by group_add_time,id";
                   // print_r($sql);
                   // 
                

                    $groupUserPayList = M()->query($sql);
//                    $groupUserPayList = $rechargeLogObj->
//                    field('id ,user_id,add_time,substr(add_time,1,10) as group_add_time,
//                    amount,recharge_no,
//                    1 as frm')->
//                    union($sql)->
//                    where($conditions)->select(); //->order('add_time')
                    ...
                    $nowTimeArr = array_keys($groupUserList);

                    while (true) {

                        if(empty($nowTimeArr[$pos])) break;

                        unset($userPayList);

                        $list[$pos]['cons_value'] = $channelOnes['cons_value'];
                        $list[$pos]['add_time']  = $now_time = $nowTimeArr[$pos];

                        $userPayList = isset($groupUserList[$now_time])?$groupUserList[$now_time]:null;

                        if(!$userPayList) continue;
                        ...
                        foreach ($userPayList as $key => $val) {
                            ...
                            if (!in_array($val['user_id'], $firstUserList)) { // 是第一次购买用户ID
                                array_push($firstUserList, $val['user_id']);
                                $userDueDetailWhere = " and add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'";
                                if (!$userDueDetailObj->where('user_id=' . $val['user_id'] . ' and id<' . $val['id'] . $userDueDetailWhere)->getField('id')) { // 检查这个时间点外该用户是否有购买(没有)
                                    $firstPayPersonSum += 1;
...
//                        else break;
                ...
                if($userInfo){
                    ...
                    foreach($userInfo as $key => $val){
                        ...
                        foreach($rechargeLogList as $k => $v){
                            ...
                            //dump($rechargeLogList['project_info']);
    ...
    public function diffBetweenTwoDays ($day1, $day2){
        $secondStart = strtotime($day1);
        $secondEnd = strtotime($day2);
        if($secondStart < $secondEnd) {
            $tmp = $secondEnd;
            $secondEnd = $secondStart;
            $secondStart = $tmp;
        }
        return (($secondStart - $secondEnd) / 86400)+1;
    }

    /**
     * 每日统计（新）
     */
    public function daily_statistics($dataTime=''){
        if(!IS_POST){
            $chn = I('get.chn', '', 'int');
            $page = I('get.p', 1, 'int');
            $start_time = I('get.st', $dataTime, 'strip_tags');
            $end_time = I('get.et', $dataTime, 'strip_tags');

            $count = 1; //每页 一个渠道号

           // if(!$start_time) $start_time = date('Y-m-d',strtotime('0 days'));
           // if(!$end_time)   $end_time = date('Y-m-d',strtotime('0 days'));
            ...
            if($start_time && $end_time){
                ...
                $channelArr = $channelArr = $channelKeyIdArr = array();
                foreach ($channelList as $c){
                    $channelArr[$c['id']] = $c['cons_value'];
                    $channelKeyArr[$c['id']] = $c['cons_key'];
                    $channelKeyIdArr[$c['cons_key']] = $c['id'];
                }

                $cacheFile = 'channel/';

                if(!$chn){ // 全部渠道
                    //$countTime= $this->diffBetweenTwoDays($end_time,$start_time); // 时间天数
                    ...
                    $sql .= "UNION ";
                    //当日注册用户数
                    $sql .="select id ,id as user_id,0,add_time,substr(add_time,1,10) as group_add_time,3 as frm  from s_user where id>0 and add_time>='" . $start_time . " 00:00:00.000000' and add_time<='" . $end_time . " 23:59:59.999000' ";
                    $sql .= "UNION ";
                    //当日注册用户数
                    $sql .="select id ,user_id,0,add_time,substr(add_time,1,10) as group_add_time,3 as frm  from s_user_channel where user_id>0 and add_time>='" . $start_time . " 00:00:00.000000' and add_time<='" . $end_time . " 23:59:59.999000' ";
//                    $sql .= "UNION all ";
//                    $sql .= "select id,user_id,0,add_time,substr(add_time,1,10) as group_add_time,2 as frm from s_user_wallet_records where type=1 and pay_status=2 and add_time>='" . $start_time . " 00:00:00.000000' and add_time<='" . $end_time . " 23:59:59.999000'  ":
                    //还款|回款
                    $sql .= "UNION ";
                    $sql .= "select id ,user_id,0,add_time,substr(add_time,1,10) as group_add_time,4 as frm  from s_user_wallet_records where  user_id>0 and add_time>='" . $start_time . " 00:00:00.000000' and add_time<='" . $end_time . " 23:59:59.999000' and type=4  ";
                    //提现
                    $sql .= "UNION ";
                    $sql .= "select id,user_id,0,add_time,substr(add_time,1,10) as group_add_time,5 as frm from s_user_wallet_records where type=2 and status=1  and add_time>='" . $start_time . " 00:00:00.000000' and add_time<='" . $end_time . " 23:59:59.999000' " ;


                    $sql  .= " ) as t  order by group_add_time,id asc";



                    $userPayLists = M()->query($sql);

                    $userPayIdArr = array();
                    foreach ($userPayLists as $userList){
                        $userPayIdArr[] = $userList['user_id'];
                    }


                    $userChannelArrRow = $channelValueArr = array();
                    if($userPayIdArr) {
                        $userIdCond['user_id'] = $userIdCondNew['id'] = array('in', $userPayIdArr);
                        $userChannelArrRow = M('user_channel')->field('user_id,channel')->where($userIdCond)->select();
                        $channelValueArr = $userObj->field('id,channel')->where($userIdCondNew)->select();
                    }

                    $userChannelArrData = array(); 
                    foreach ($userChannelArrRow as $uRow) { 
                        $userChannelArrData[$uRow['user_id']]  = $uRow['channel'];
                    }  

                    foreach ($channelValueArr as $uArr) {
                        $userChannelArrData[$uArr['id']]  = $uArr['channel'];
                    }    


                    $groupUserPayList = array();
                    foreach ($userPayLists as $userList){
                        $userChannel = $userChannelArrData[$userList['user_id']];
                        $channel_id = isset($channelKeyIdArr[$userChannel])?$channelKeyIdArr[$userChannel]:0;
                        if($channel_id>0) {
                            if (isset($userList['group_add_time'])) {
                                $groupAddTime = $userList['group_add_time'];
                                $groupUserPayList[$groupAddTime][] = $userList;
                            }
                        }
                        // $channelValueArr = $userObj->field('channel_id,channel')->where('id='.$userList['user_id'])->find();

                        // if($channelValueArr['channel_id']>0) {
                        //     if (isset($userList['group_add_time'])) {
                        //         $groupAddTime = $userList['group_add_time'];
                        //         $groupUserPayList[$groupAddTime][] = $userList;
                        //     }
                        // }else{
                        //    // $constantValueArr = M('constant')->join('s_user_channel u on s_constant.cons_value = u.channel','left')->field('s_constant.id as channel_id,u.channel')->where('u.user_id='.$userList['user_id'])->find();  
                        //     $userChannel = M('user_channel')->where('user_id='.$userList['user_id'])->getField('channel');
                        //     $channel_id = isset($channelKeyIdArr[$userChannel])?$channelKeyIdArr[$userChannel]:0;
                    ...
                    $counts = count($groupUserPayList); 
                    $PageObj = new \Think\Page($counts, $count);
                    $show = $PageObj->show();
                    ...
                    foreach ($groupUserPayList as $time=> $g){
                        $pageArr[] = $time;
                    }
                    ...
                    $add_time = isset($pageArr[$page-1])?$pageArr[$page-1]:0;
                    $array[] = $add_time;
                    foreach($array as  $add_time_value) {
                         if(!$add_time_value) continue;

                         $now_time = $add_time_value;//date('Y-m-d', strtotime("$end_time  -$min day"));

                           unset($rows);
                           unset($cond);
                           $rows['datetime'] = $now_time;

                           $userPayList = isset($groupUserPayList[$now_time])?$groupUserPayList[$now_time]:array();
                           ...
                           $userIdArr = $whereUserIdArr = $userChannelObjArr = array();
                           foreach ($userPayList as $userArr){
                               if($userArr['user_id']>0) {
                                   $userIdArr[] = $userArr['user_id'];
                               }
                           }

                           $userIdArr = array_unique($userIdArr);

                           $userChannelObjArr = array();

                           if($userIdArr) {
                               $whereUserIdArr['id'] = array('in', $userIdArr);
                              // $whereUserIdArr['channel_id'] = array('neq', 0);
                               $userChannelArr = $userObj->field('channel_id ,id')->where($whereUserIdArr)->select();

                               foreach ($userChannelArr as $channelId) {
                                   $userChannelObjArr[$channelId['channel_id']][] = $channelId['id'];
                               }
                           }   

                           foreach ($userChannelObjArr as $channelKey=>$channelIdArr){
                               $cacheDoc = $cacheFile.$now_time.'/channel-'.$channelKey;
                               if($now_time<date('Y-m-d')){
                                   $rows = F($cacheDoc); 
                                   if($rows){
                                        $rows['datetime'] = $now_time;
                                        array_push($list, $rows); continue;
                                   }
                               }
                               ...
                               $arrayUserPayList = array();
                               foreach ($userPayList as $u){
                                   $uid = $u['user_id'];
                                   if(in_array($uid,$channelIdArr)) {
                                       $arrayUserPayList[] = $u;
                                   }
                               }
                              // $UserListUIdArr = array();//所有uid

                               foreach ($arrayUserPayList as $key => $val) { 
                                   //$UserListUIdArr[] = $val['user_id'];
                                  // $userIdOne = $val['user_id'];
                                  // if(!in_array($userIdOne,$channelIdArr)) continue;
                                   if ($val['frm'] == 1) {
                                       $totleCount += 1;

                                       if (!in_array($val['user_id'], $firstUserList) ) { // 是第一次购买用户ID
                                           $firstPayCount += 1;  //首投次数
                                           array_push($firstUserList, $val['user_id']);

                                           $isTrue = true;
                                           if($firstPayCount > 5)  { $firstPayCount -= 1;$isTrue = false; }// 首投最多 5 次记录

                                           if($isTrue) {
                                               $totlePayPersonCount += 1;
                                               $userDueDetailWhere = " and add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'";
                                               if (!$userDueDetailObj->where('user_id=' . $val['user_id'] . ' and id<' . $val['id'] . $userDueDetailWhere)->getField('id')) {
                                   ...
                                   if (!in_array($val['user_id'], $userIdList)) {
                                       ...
                                       $_cond .= " and add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'" ; // and status_new=2

                                       $userDueDetailArr = M('userWalletRecords')->field('value,user_id ')->where($_cond .' and type=4')->select();
                               ...
                               $consKey = $channelKeyArr[$channelKey];
                               $UserListUIdArr = $this->statisticsModel->channelUserId($channelKey,$consKey);
                               //流失用户
                               $lossUserCount = $this->statisticsModel->lossUserCount($now_time,$UserListUIdArr);
                               //加个渠道id
                               if ($channelKey) {
                                   $channelIdWhere = " and channel_id=" . $channelKey;
                               }


                               $consValue =isset($channelArr[$channelKey]) ? trim($channelArr[$channelKey]) : '';



                            //   $channelIdChannelWhere = ' and channel != \'\' and channel like \''.$consKey.'%\'';// $consValue
                            //   $channelIdChannelWhere = "add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'" . $channelIdChannelWhere ;


                               //当日注册用户数
                               $totleRegisteredUsers = $this->statisticsModel->regUserTotle($consKey,$now_time,$channelKey);

//                               $totleRegisteredUsers = $userObj->field('id')->
//                               union("select count(id)  AS tp_count  from s_user_channel where ".$channelIdChannelWhere ." LIMIT 1")->
//                               where("add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'" . $channelIdWhere)->count();
                               ...
                               $rows['totle_pay_sum'] = $totlePaySum; //$firstPaySum + $nextPaySum;  // 累计投资总额 //$item['first_pay_sum'] + $item['next_pay_sum']
                   ...
                   // $sqlChn = " where  user_id in (select id from s_user where channel_id=" . $chn . " OR id IN (SELECT user_id FROM s_user_channel WHERE channel LIKE '".$cons_key."%' ) ) ";
                    ...
                    if(in_array($cons_key,$this->consKeyArray)) { // 特殊处理
                        $cond[] = "user_id in (select id from s_user where channel_id=".$chn." OR id IN (SELECT user_id FROM s_user_channel WHERE channel = '".$cons_key."' ) )";
                    }else{
                        $cond[] = "user_id in (select id from s_user where channel_id=".$chn." OR id IN (SELECT user_id FROM s_user_channel WHERE channel LIKE '".$cons_key."%' ) )";
                    }
                  ...
                  //  $sql.= "select id,user_id,0,add_time,substr(add_time,1,10) as group_add_time,2 as frm from s_user_wallet_records where type=1 and pay_status=2 and add_time>='".$start_time." 00:00:00.000000' and add_time<='".$end_time." 23:59:59.999000' ";
                    ...
                    $sql.= " ) as t " .$sqlChn. " order by group_add_time,id asc";  


                    //print_r($sql);

                    $userPayListArr = M()->query($sql);



                    $userPayListGroupArr = array();
                    foreach ($userPayListArr as $u){
                        $userPayListGroupArr[$u['group_add_time']][]  = $u;
                    }

                    foreach ($userPayListGroupArr as $time=>$itemUser ) {
                        $now_time = $time;

                        $cacheDoc = $cacheFile.$now_time.'/channel-'.$chn;
                        if($now_time<date('Y-m-d')){
                            $rows = F($cacheDoc);
                            if($rows){
                                $rows['datetime'] = $now_time;
                        ...
                        if ($userPayListGroupArr[$now_time]) {
                            unset($rows);
                            unset($cond);
                            $rows['datetime'] = $now_time;
                            ...
                            $userPayList = isset($userPayListGroupArr[$now_time]) ? $userPayListGroupArr[$now_time] : array();

                            $UserListUIdArr= array();//所有用户uid;
                            ...
                            foreach ($userPayList as $key => $val) {
                                ...
                                if (!in_array($val['user_id'], $userIdList)) { //  用户ID数组(防止统计重复用户)
                                    ...
                                    $userDueDetailArr = M('user_wallet_records')->field('value,user_id ')->where($_cond . ' and type=4')->select();
                            ...
                            $consKey = $channelKeyArr[$chn];
                            $UserListUIdArr = $this->statisticsModel->channelUserId($chn,$consKey);
                            //流失用户
                            $lossUserCount = $this->statisticsModel->lossUserCount($now_time,$UserListUIdArr);

                            $channelIdChannelWhere = ' and channel != \'\' and channel like \''.$consValue.'%\'';
//                          $channelIdChannelWhere = "add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'" . $channelIdChannelWhere ;
//

                            //当日注册用户数

                           // $totleRegisteredUsers = $this->statisticsModel->regUserTotle($channelIdChannelWhere,$now_time,$chn);
                            $totleRegisteredUsers = $this->statisticsModel->regUserTotle($consKey,$now_time,$chn);

//                            $totleRegisteredUsers = $userObj->
//                            union("select count(id)  AS tp_count  from s_user_channel where ".$channelIdChannelWhere ." LIMIT 1")->
//                            where("channel_id=" . $chn . " and add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'")->count();
                ...
                $sumParams = array(
                    'sumTotleRegisteredUsers'=>$sumTotleRegisteredUsers,
                ...
                $this->assign('sumParams', array());//$sumParams
                $this->assign('show',$show);
                $this->assign('list', $list);
            }
...
//                    where($conditions)->select(); //->order('add_time')
                       ...
                       // if($max > $counts) $max = $counts;
                        ...
                        for($i = $min; $i < $max; $i++){
                            ...
                            $cons_key = $userPayListRow['cons_key'];
                            ...
                            unset($userPayList['channel_id'],$userPayList['cons_value'],$userPayList['cons_key'],$userPayList['group_add_time']);
...
//                    where($conditions)->select(); //->order('add_time')
                    ...
                    while (true) {
                        ...
                        //$unionSql = "select count(id) as tp_count   from s_user_channel where channel LIKE '".$cons_key."%' and  user_id>0 and add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000' LIMIT 1 ";
                        //$list[$pos]['user_count'] = $userObj->union($unionSql)->where("channel_id=" . $chn . " and add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'")->count();
                        ...
                        ///$list[$pos]['user_count'] = $userObj->where()->count();
                        $list[$pos]['user_download'] = $this->download($chn,$now_time); // 下载量
//                        if ($now_time != $start_time) $now_time = date('Y-m-d', strtotime("$now_time -1 day"));
//                        else break;
...
//                    $sql .= "select id,user_id,0,add_time,substr(add_time,1,10) as group_add_time,2 as frm from s_user_wallet_records where type=1 and pay_status=2 and add_time>='" . $start_time . " 00:00:00.000000' and add_time<='" . $end_time . " 23:59:59.999000'  ":
                ...
                foreach ($userPayListGroupArr as $timeUser => $userPayList) {
                    $now_time = $timeUser;
                    $cacheDoc = $cacheFile.$now_time.'/channel-'.$chn;
                    if($now_time<date('Y-m-d')){
                        $rows = F($cacheDoc);
                    ...
                    if ($userPayListGroupArr[$now_time]) {
                            ...
                            if ($val['frm'] == 1) {
                                ...
                                if (!in_array($val['user_id'], $firstUserList)) { // 是第一次购买用户ID
                                    ...
                                    if ($firstPayCount <= 5) { //首投最多5次
                                        ...
                                        $totlePayPersonCount += 1;
                                        $userDueDetailWhere = " and due_time>='" . $now_time . " 00:00:00.000000' and due_time<='" . $now_time . " 23:59:59.999000'";
                                        if (!$userDueDetailObj->where('user_id=' . $val['user_id'] . ' and id<' . $val['id'] . $userDueDetailWhere)->getField('id')) {
                                            $firstPayPersonSum += 1;
                                            $firstPaySum += $val['due_capital'];
                                        } else {
                       ...
                       // $channelIdChannelWhere = ' and channel != \'\' and channel like \''.$consKey.'%\'';
                       // $channelIdChannelWhere = "add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'" . $channelIdChannelWhere ;
...
//                        where("channel_id=" . $chn . " and add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'")->count();
        ...
        $this->excelModel->excelFile($list,true);
    }
        ...
        foreach ($groupArr as $date_time =>$gArr) {
                ...
                foreach ($gArr as  $chnId => $userPayRows) {
                    ...
                    foreach ($userPayRows as $key => $val) {
                        $UserListUIdArr[] =  $varUserId = $val['user_id'];
                  ...
                  //  if (!$channelDateUserArr) continue;
                    ...
                    $consKey = $channelKeyArr[$chnId];
                    $UserListUIdArr = $this->statisticsModel->channelUserId($chnId,$consKey);
                    //流失用户
                    $lossUserCount = $this->statisticsModel->lossUserCount($now_time,$UserListUIdArr);
                ...
                //    $channelIdChannelWhere = "add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'" . $channelIdChannelWhere ;
                    ...
                    $totleRegisteredUsers = $this->statisticsModel->regUserTotle($consKey,$now_time,$chnId);
//                    $totleRegisteredUsers = $userObj->field('id')->
//                    union("select count(id)  AS tp_count  from s_user_channel where ".$channelIdChannelWhere ." LIMIT 1")->
//                    where("add_time>='" . $now_time . " 00:00:00.000000' and add_time<='" . $now_time . " 23:59:59.999000'" . $channelIdWhere)->count();
    ...
    private function daily_statistics_field(){
        return array(
            ...
            'first_withdraw_sum.f',
            'total_withdraw_user_sum',
            'total_withdraw_user_count',
            'total_withdraw_pay_sum.f',
            'total_repayment_user',
            'total_repayment_pay_sum.f',
            'loss_users',
            'silent_users'
        );
    }
      ...
      //  error_reporting(E_ALL ^ E_NOTICE);
        ...
        if ($_FILES) {
           ...
           // $config = array();
            ...
            if($info) {
                $nameType = $info['xls']['ext'];

                $xlsPath = $info['xls']['savepath'].$info['xls']['savename'];

                //$ossPath = 'Uploads/focus/'.$image;
                $xls = C('localPath').$xlsPath;
...
//                $this->ajaxReturn(array('status'=>0,'info'=>'格式错误！'));
//            }



            if(!in_array($nameType, array('xls','xlsx'))){
                $this->ajaxReturn(array('status'=>0,'info'=>'格式错误！'));
            }
            // 设置用户定义的错误处理函数
            $extension = strtolower( pathinfo($xls, PATHINFO_EXTENSION) );
            ...
            $objPHPExcel = $_ReadExcel->load($xls);
        }catch (Exception $e){
        ...
        for($j=2;$j<=$highestRow;$j++) {
           ...
           // $add_data['date_time'] = date('Y-m-d',strtotime($strs[1]));
            if(strpos($strs[1],'-')){
                ...
                $n = intval(($strs[1] - 25569) * 3600 * 24); //转换成1970年以来的秒数
            }
            $add_data['date_time'] = date('Y-m-d',$n);
        ...
        $channelDownloadChannelIdNewTimeArr = $channelDownloadChannelIdNewArr = $channelDateTimeArr = array();
        foreach ($channelDownloadChannelIdArr as $downloadArr){
           ...
           // $channelDateTimeArr[] = $downloadArr['date_time'];
            $channelDownloadChannelIdNewTimeArr[] = $downloadArr['channel_id'] .'/'.$downloadArr['date_time'];

        }


        if($count>100) $dateChannel = array_slice($dateChannel, 0,100);
        foreach ($dateChannel as $d){
            $channel = trim($d['channel']);
            $dateTime = trim($d['date_time']);
            $channelId = isset($channelArray[$channel])?$channelArray[$channel]:0;
           // $channelDateTime = $channelArray[$dateTime];
            $d['channel_id'] = $channelId;
            //if(!in_array($channelId,$channelDownloadChannelIdNewArr) && !in_array($dateTime,$channelDateTimeArr)){ ///
            $channelIdTime =  $channelId . '/' . $dateTime; 
            if(!in_array($channelIdTime,$channelDownloadChannelIdNewTimeArr)){ ///
                if($channelId) {
                    $addData[] = $d;
                }
            }
     ...
     * 每日统计(旧)
     */
    public function daily_statistics_old(){
        if(!IS_POST){
            $chn = I('get.chn', '', 'int');
            $start_time = I('get.st', '', 'strip_tags');
            $end_time = I('get.et', '', 'strip_tags');
...
//                $rechargeLogObj = M('RechargeLog');
                ...
                if(!$chn){ // 全部渠道
                    ...
                    while(true){
                        ...
                        $sumRegistered_users += $totleRegisteredUsers;
                        $rows['new_second_pay_person_count'] = $newSecondPayPersonCount;
                        $rows['totle_pay_person_count'] = $totlePayPersonCount;
                        $rows['totle_count'] = $totleCount;
                        $rows['first_pay_person_sum'] = $firstPayPersonSum;
                ...
                $sumParams = array(
                    'sumNewSecondPayPersonCount' => $sumNewSecondPayPersonCount,
                    'sumTotlePayPersonCount' => $sumTotlePayPersonCount,
                    'sumRegistered_users'=>$sumRegistered_users,
                    'sumTotleCount' => $sumTotleCount,
                    'sumFirstPayPerson' => $sumFirstPayPerson,
                    'sumNextPayPerson' => $sumNextPayPerson,
                    'sumFirstPay' => $sumFirstPay,
            ...
            if($start_time) $query .= '/st/'.$start_time;
            if($end_time) $query .= '/et/'.$end_time;
            redirect(C('ADMIN_ROOT').'/statistics/daily_statistics_old'.$query);
        }
    }
