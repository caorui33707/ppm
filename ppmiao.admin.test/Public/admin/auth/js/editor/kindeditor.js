...
KE.util = {
	...
	getMediaImage : function(id, type, attrs) {
		...
		var html = '<images class="' + className + '" src="' + KE.g[id].skinsPath + 'common/blank.gif" ';
	...
	innerHtml : function(el, html) {
		if (KE.browser.IE) {
			el.innerHTML = '<images id="__ke_temp_tag__" width="0" height="0" />' + html;
	...
	pasteHtml : function(id, html, isStart) {
		var g = KE.g[id];
		var imgStr = '<images id="__ke_temp_tag__" width="0" height="0" />';
...
KE.dialog = function(arg){
	...
	this.show = function() {
		...
		loadingImg.className = 'ke-loading-images';
...
KE.create = function(id, mode) {
	...
	var leftImg = KE.$$('span');
	leftImg.className = 'ke-bottom-left-images';
	...
	rightImg.className = 'ke-bottom-right-images';
	if (KE.g[id].config.resizeMode == 0 || mode == 1) {
...
KE.plugin['emoticons'] = {
		...
		function createEmoticonsTable(pageNum) {
			...
			for (var i = 0; i < rows; i++) {
				...
				for (var j = 0; j < cols; j++) {
					...
					var span = KE.$$('span');
					span.className = 'ke-plugin-emoticons-images';
	...
	exec : function(id, num) {
		...
		var html = '<images src="' + src + '" kesrc="' + src + '" border="0" alt="" />';
		KE.util.insertHtml(id, html);
		this.menu.hide();
		KE.util.focus(id);
	}
...
KE.plugin['image'] = {
	...
	insert : function(id, url, title, width, height, border, align) {
		var html = '<images src="' + url + '" kesrc="' + url + '" ';
