...
function stageInit(){
	$.post(ROOT + "/Common/getProductStageByType", function(msg){
        if(msg.status){
            $('input[name=stage]').val(msg.info);
            getTitle();
            create_contract_no();
...
/*
            <case value="2">电子银行承兑汇票</case>
            <case value="3">银行承兑汇票</case>
            <case value="4">电子商业承兑汇票</case>
            <case value="5">商业承兑汇票</case>
 */
		...
		$.post(ROOT + "/Common2/getContractInfo", {contract_no: 'ppm'+$('input[name=stage]').val()}, function(msg){
            if(msg.status){
               ...
               // $('input[name=accepting_bank]').val(msg.info.acceptor);
                ...
                $('input[name=guaranty_institution]').val(msg.info.guaranty_institution + '担保');
...
function count_earn(){ // 计算项目可获利
    ...
    //var _contract_counter_fee = $("input[name=contract_counter_fee]").val();
    var _targetPer = $("input[name=contract_interest]").val();
    var _user_platform_subsidy = $("input[name=user_platform_subsidy]").val();	
    //$("#corporation_earnings").val(format(((_targetPer+_contract_counter_fee)-(_userPer+_user_platform_subsidy))*_totle*_days/(365*100), 2));
}
...
$(document).ready(function(){
    ...
    $("#frmMain").Validform({ // 表单验证
        ...
        callback: function(form){
            if($("#start_time").val() == '' || $("#end_time").val() == ''){
                layer.alert('请选择起止日期!', -1, function(){
