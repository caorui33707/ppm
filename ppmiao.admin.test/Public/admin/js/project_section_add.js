...
function changeType(_obj){ // 改变类型
    ...
    if(_type > 0){
        if(_type == 109 || _type == 110 || _type == 104) { // 基金类(包括打新股)
            ...
            window.location.href = ROOT + '/project/add_itype/' + _type;
        }else{  // 其他
...
function chooseFund(){ // 选择基金页面
    window.open(ROOT + "/fund/select_page", 'fundSelectPage', 'height=600,width=800,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
}
