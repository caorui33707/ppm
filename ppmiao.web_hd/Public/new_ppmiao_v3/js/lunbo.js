  $(function(){
	  ...
	  //自动控制图片轮播
	  var i=0; //初始i=0
	  var t=setInterval(move,2000);  //设置定时器，1.5秒切换下一站轮播图
	  //向左切换函数
	  function moveL(){
		  i--;
		  if(i==-1){
	  ...
	  $(".out").hover(function(){
		  ...
		  t=setInterval(move,11000);  //鼠标移开时定时器继续
	  });


      //捐款轮播图
      var $div = $(".information_notice>div");
      var height = $div.find("p").height() + 10;
      var count = $div.find("p").length;
      var timer = setInterval(slide, 1000);

      $div.append($div.find(":lt(18)").clone(true));

      var staticTop = $div.position().top;
      ...
      function slide() {
          ...
          index++;
          if(index == count){
              $div.offset({top:staticTop});
              index = -1;
          }
          var t = $div.offset().top;
          $div.animate({
              top: t - height - staticTop
          },1000,"linear");
      }
