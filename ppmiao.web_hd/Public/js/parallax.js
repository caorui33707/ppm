...
function fnLoad() {
	...
	var boxHeight = (windowHeight-78)>570 ? (windowHeight-78) : 570;
	...
	document.getElementsByTagName("body")[0].style.height = (line.lineHeight + 78) * line.scrollRatio + "px";
	...
	var speedTop = 800;
	var scale = (line.lineHeight + 78 - windowHeight)/((line.lineHeight + 78) * line.scrollRatio - windowHeight);


	//加入我们跳转
	$(document).ready(function(){
        var index = sessionStorage.getItem("turn_and_go_index");
        //document.getElementsByTagName('body')[0].scrollTop = index * line.scrollRatio * boxHeight;
	    if(typeof(index)!="undefine"){
            $('window').scrollTop(0, index * line.scrollRatio * boxHeight);
	        //window.scroll(0, index * line.scrollRatio * boxHeight);
	        //var index = sessionStorage.removeItem("turn_and_go_index");
	    }
	});
...
//		}]
	...
	function scrollFunc(e) {
		var e = e || event;
		var wrapTop = 78 - document.getElementById("wrapBox").offsetTop;
		var direct;
		var endWrapTop;
		...
		window.scroll(0, endWrapTop/(line.lineHeight + 78 - windowHeight)*((line.lineHeight + 78) * line.scrollRatio - windowHeight));
	}
	...
	window.onscroll = function() {
		...
		// }
		endTop = top;
		//$wrapBox.css({"top": -top2+78});
		var wrapTop = top*(line.lineHeight + 78 - windowHeight)/((line.lineHeight + 78) * line.scrollRatio - windowHeight);//内容改变top值

		//$wrapBox.css({"top": -wrapTop+60});
		$wrapBox.stop().animate({"top": -wrapTop+78}, 1300);//#wrap 滚动
		//$wrapBox.stop().animate({"top": -wrapTop+78},{queue:false, duration: 1100});
...
function boxGoIn_1() {
	 box1In = false;
	$(".b1_02").stop().animate({left:460, opacity: 1}, 1200, function() {
		$(".b1_03_btn").addClass('anima');
	});
	$(".b1_03").stop().animate({top: 285, opacity: 1}, 1200, function() {
		$(".b1_04").stop().animate({top: 68, opacity: 1}, 1000, function() {
			$(".b1_05").stop().animate({top: 28, opacity: 1}, 1000);
		});
	});
}
function boxGoOut_1() {
    box1In = true;
    $(".b1_03").stop().animate({top: 285, opacity: 0}, 1500);
    $(".b1_04").stop().animate({top: 68, opacity: 0}, 1500);
    $(".b1_05").stop().animate({top: 28, opacity: 0}, 1500);
    $(".b1_02").stop().animate({left: 380, opacity: 0}, 500, function() {
...
function boxGoIn_2() {
	 box2In = false;
	$(".b2_01").stop().animate({top: 20, opacity: 1}, 1200);
	$(".b2_02").stop().animate({left: 500, opacity: 1}, 1200);
	$(".b2_03").stop().animate({opacity: 1}, 1000);
	$(".b2_04").stop().animate({left: 1200, opacity: 1}, 1200);
}
function boxGoOut_2() {
	 box2In = true;
	$(".b2_01").stop().animate({top: 40, opacity: 0}, 500);
	$(".b2_02").stop().animate({left: 300, opacity: 0}, 500);
	$(".b2_03").stop().animate({opacity: 0}, 300);
	$(".b2_04").stop().animate({left: 1500, opacity: 0}, 500);
}
...
function boxGoIn_3() {
	...
	$(".b3_img01").addClass("transform");
	$(".b3_img01").stop().animate({opacity: 1}, 1500);
	$(".b3_tl").stop().animate({opacity: 1}, 1500);
	$(".b3_01").stop().animate({top: 60, opacity: 1}, 1200);
	box3Now = 0;
	var aJson = [{left: 440, top:200,opacity: 1},
		{left: 430, top:465, opacity: 1},
		{left: 1010, top:180, opacity: 1},
		{left: 1020, top:420, opacity: 1},
	];

	for (var i=1; i<7; i++) {
		$(".b3_txt0" + i).stop().animate(aJson[i-1], 1500);
	}
...
function boxGoOut_3() {
	box3In = true;
	clearInterval(box3Timer);
	$(".b3_img01").stop().animate({opacity: 0}, 1500, function() {
		$(".b3_img01").removeClass("transform");
	});
	$(".b3_01").stop().animate({top: 40, opacity: 0}, 500);
	box3Now = 2;
	...
	// 	{left: 139, opacity: 0}
	// ];
	var aJson = [{left:580,top:300,opacity: 0},
		{left: 580, top: 375, opacity: 0},
		{left: 860,  top: 280, opacity: 0},
		{left: 870, top: 363, opacity: 0},
	];

	for (var i=1; i<7; i++) {
		$(".b3_txt0" + i).stop().animate(aJson[i-1], 1000);
	}
	// for (var i=0; i<box3Order[box3Now].length; i++) {
...
function boxGoIn_5() {
	box5In = false;
	clearTimeout(box5Timer);
	$(".b5_img01").stop().animate({top: 0, opacity: 1}, 1350);
	box5Timer = setTimeout(function() {
		$(".b5_txt01").stop().animate({top: 105, opacity: 1}, 1200);
		box5Timer = setTimeout(function() {
			$(".b5_txt02").stop().animate({top: 140, opacity: 1}, 1050);
			box5Timer = setTimeout(function() {
				$(".b5_txt03").stop().animate({top: 180, opacity: 1}, 900);
				box5Timer = setTimeout(function() {
					$(".b5_txt04").stop().animate({top: 220, opacity: 1}, 750);
					box5Timer = setTimeout(function() {
						$(".b5_txt05").stop().animate({top: 250, opacity: 1}, 600);
						$(".b5_txt06").stop().animate({top: 460, opacity: 1}, 600);
						box5Timer = setTimeout(function() {
							$(".b5_txt07").stop().animate({top: 515, opacity: 1}, 450);
						}, 150);
					}, 150);
				}, 150);
			}, 150);
...
function boxGoOut_5() {
	...
	clearTimeout(box5Timer);
	box5Timer = setTimeout(function() {
		$(".b5_txt07").stop().animate({top: 1340, opacity: 0}, 600);
		box5Timer = setTimeout(function() {
			$(".b5_txt06").stop().animate({top: 1258, opacity: 0}, 525);
			box5Timer = setTimeout(function() {
				$(".b5_txt05").stop().animate({top: 1176, opacity: 0}, 450);
				$(".b5_txt04").stop().animate({top: 1235, opacity: 0}, 450);
				box5Timer = setTimeout(function() {
					$(".b5_txt03").stop().animate({top: 1000, opacity: 0}, 375);
					box5Timer = setTimeout(function() {
						$(".b5_txt02").stop().animate({top: 1012, opacity: 0}, 300);
						$(".b5_txt01").stop().animate({top: 1195, opacity: 0}, 300);
						box5Timer = setTimeout(function() {
							$(".b5_img01").stop().animate({top: 930, opacity: 0}, 500);
						}, 75);
					}, 75);
				}, 75);
			}, 75);
...
//	$(".b6_01").stop().animate({left: 1220, opacity: 1}, 1500);
	$(".se04Img01").stop().animate({opacity: 1}, 1200, function() {
		$(".b4_txt").stop().animate({opacity: 1}, 800, function() {
...
function boxGoOut_4() {
	...
	$(".b4_txt").stop().animate({opacity: 0}, 500);
	//$(".b6_tl").stop().animate({opacity: 0}, 500);
	$(".se04Img01").stop().animate({opacity: 0}, 500);
}
var box6Timer;
function boxGoIn_6() {
	box6In = false;
	$(".b6_img01").stop().animate({opacity: 1}, 1200, function() {
		$(".b6_img02").stop().animate({opacity: 1}, 800);
		$(".b6_img03").stop().animate({opacity: 1}, 800);
	});
}

function boxGoOut_6() {
	box6In = true;
	$(".b6_img03").stop().animate({opacity: 0}, 500);
	$(".b6_img02").stop().animate({opacity: 0}, 500);
	$(".b6_img01").stop().animate({opacity: 0}, 500);
}
...
function boxGoIn_7() {
	...
	$(".b7_txt01").stop().animate({top: 135, opacity: 1}, 1350);
	box7Timer = setTimeout(function() {
		$(".b7_img03").stop().animate({opacity: 1}, 800);
		$(".b7_txt02").stop().animate({top: 160, opacity: 1}, 1200);
		box7Timer = setTimeout(function() {
			$(".b7_txt03").stop().animate({top: 170, opacity: 1}, 1050);
			box7Timer = setTimeout(function() {
				$(".b7_txt04").stop().animate({top: 190, opacity: 1}, 900);
				box7Timer = setTimeout(function() {
					$(".b7_txt05").stop().animate({top: 300, opacity: 1}, 750);				
				}, 1200);
			}, 1200);
		}, 1200);
	}, 1200);
...
function boxGoOut_7() {
	...
	clearTimeout(box7Timer);
	box7Timer = setTimeout(function() {
		$(".b7_img03").stop().animate({opacity: 0}, 500);
		$(".b7_txt05").stop().animate({ opacity: 0}, 600);
		box7Timer = setTimeout(function() {
			$(".b7_txt04").stop().animate({opacity: 0}, 525);
			box7Timer = setTimeout(function() {
				$(".b7_txt03").stop().animate({opacity: 0}, 450);
				box7Timer = setTimeout(function() {
					$(".b7_txt02").stop().animate({opacity: 0}, 375);
					box7Timer = setTimeout(function() {
						$(".b7_txt01").stop().animate({opacity: 0}, 300);
					}, 75);
				}, 75);
			}, 75);
		}, 75);
...
var loadImg = {
	path: './Public/images/',
	imgs: {
		'.b1_bg': ['b1_01.png','screen1.png','screen2.png','screen3.png','se01Img01.png','erweima.jpg','androidImg.png','iphoneImg.png','sprite.png'],
		'.b2_bg': ['se02Img01.png','se02list01.png','se02list02.png','se02list03.png','se02list04.png','se02list01Selected.png','se02list02Selected.png','se02list03Selected.png','se02list04Selected.png'],
		'.b3_bg': ['se03Img01.png','b3_icon01.png','b3_icon02.png','b3_img01.png','b3_txt01.png','b3_txt02.png','b3_txt03.png','b3_txt04.png'],
		'.b4_bg': ['se04Img01.png','payh.png','jsyh.png'],
		'.b5_bg': ['b5_txt01.png','b5_txt02.png','b5_txt03.png','b5_txt04.png'],
		'.b6_bg': ['b6_01.png','b6_img01.png','b6_img02.png'],
		'.b7_bg': ['b7_img01.png','b7_img02.png','b6_img02.png']
	},	
