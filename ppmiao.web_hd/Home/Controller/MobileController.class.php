...
//        var_dump($userDueDetailInfo);
        ...
        if($add_time > $up_time){
            if(strpos($_SERVER['HTTP_USER_AGENT'], 'Android') && ($versionNo >= 100 && $versionNo <= 124)){
                ...
                if($userDueDetailInfo['contractid'] != ''){
    ...
    */
    ...
    public function bank_limit(){
        ...
        if(!$app_ver) {
            ...
            $fied = 'bank_name,limit_times,limit_day,limit_month,small_icon';
            $list = M('Bank')->field($fied)->select();
            foreach ($list as $k=>$v){
                $list[$k]['limit_times'] = $v['limit_times']/10000;
                $list[$k]['limit_day'] = $v['limit_day']/10000;
                $list[$k]['limit_month'] = $v['limit_month']/10000;
            }
