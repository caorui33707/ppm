...
class IndexController extends BaseController {
    ...
    public function product(){
        ...
        //进行中的
        $project_list = M('Project')->field('id,title,monthly_increase_group,amount,duration,user_interest,money_min,new_preferential,percent,end_time')
        ...
        if($project_list) {
            foreach ($project_list as $val) {
				...
				$val['day'] = $this->count_days($val['end_time']);
				if($val['new_preferential']==9){ // 月月加薪
                    $user_interest = $val['user_interest'];
                    $amount_value =M('monthly_increase_interest_config')->where(array('group_id'=>$val['monthly_increase_group']))->getField( 'max(amount_value)');

                    $val['user_interest'] = $user_interest.'~'.sprintf('%.2f',($user_interest+$amount_value) );
                }
