...
function selected(id,categoryId,vaule) {
	...
	if($('#redbag' + id).css('backgroundImage').indexOf('prize.png')>0){
		...
		$('.payfor__money').empty().html(realAmount);
		$("#redbag").empty().html('未选择红包('+redNumber+'个可用)');
	}
}
...
function loadRed(){
	...
	$.ajax({
        ...
        success: function (json) {
        	...
        	if(json.status == 1){
        		var s = '';
        		redNumber = json.couponLisOn.length;
        		...
        		for(var item in json.couponLisOn) {
        			...
        			s = '<div id="redbag'+json.couponLisOn[item].id+'" class="prize" style="position:relative;"onclick="selected('+json.couponLisOn[item].id+','+json.couponLisOn[item].categoryId+','+v+')">';
                    s += '<div class="prize__money">'+v+u+'</div><div class="prize__name">'+json.couponLisOn[item].title+'</div>';
                    s += '<div style="position:absolute;top:36px;"><div class="prize__num ">投资金额≥'+json.couponLisOn[item].minInvest+'元</div>';
                    s += '<div class="prize__date">投资期限≥'+json.couponLisOn[item].minDue+'天</div><div class="prize__active">'+tagToType(json.couponLisOn[item].applyTag)+'</div><div class="prize__time">有效期：'+formatDateTime(json.couponLisOn[item].valiteTime)+'至'+formatDateTime(json.couponLisOn[item].expireTime)+'</div></div></div>';
                    html.push(s);
        		}
